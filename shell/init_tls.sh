...
# 恢复配置
resetNginxConfig(){
    `cp -Rrf /tmp/wangyu0829/nginx/nginx.conf /etc/nginx/nginx.conf`
    rm -rf /etc/nginx/conf.d/5NX2O9XQKP.conf
    echoColor green "\n恢复配置完毕"
}
# 备份
bakConfig(){
    mkdir -p /tmp/wangyu0829/nginx
    `cp -Rrf /etc/nginx/nginx.conf /tmp/wangyu0829/nginx/nginx.conf`
}
# 安装证书
installTLS(){
    ...
    ~/.acme.sh/acme.sh --installcert -d ${domain} --fullchainpath /tmp/wangyu0829/nginx/${domain}.crt --keypath /tmp/wangyu0829/nginx/${domain}.key --ecc >/dev/null
    if [[ -z `cat /tmp/wangyu0829/nginx/${domain}.key` ]]
    then
        echoColor red "证书key生成失败，请重新运行"
        resetNginxConfig
        exit
    elif [[ -z `cat /tmp/wangyu0829/nginx/${domain}.crt` ]]
    then
    ...
    echoColor green "证书目录/tmp/wangyu0829/nginx"
    ls /tmp/wangyu0829/nginx
...
init(){
    ...
    echoColor green "   12.Github[https://github.com/wangyu0829]"
    echoColor red "=============================="
