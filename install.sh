...
allowPort() {
	...
	if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
		...
		if ! iptables -L | grep -q "$1(wangyu0829)"; then
			updateFirewalldStatus=true
			iptables -I INPUT -p tcp --dport "$1" -m comment --comment "allow $1(wangyu0829)" -j ACCEPT
		fi
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/wangyu0829/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
installV2Ray() {
	...
	if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
		if [[ "${selectCoreType}" == "2" ]]; then
			...
			version=$(curl -u USERNAME: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 -s https://api.github.com/repos/v2fly/v2ray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | grep -v 'v5' | head -1)
		else
...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		version=$(curl -u USERNAME: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 -s https://api.github.com/repos/XTLS/Xray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | head -1)
...
v2rayVersionManageMenu() {
	...
	if [[ "${selectV2RayType}" == "1" ]]; then
		...
		curl -u USERNAME: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 -s https://api.github.com/repos/v2fly/v2ray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | grep -v 'v5' | head -5 | awk '{print ""NR""":"$0}'

		echoContent skyBlue "--------------------------------------------------------------"
		read -r -p "请输入要回退的版本:" selectV2rayVersionType
		version=$(curl -u USERNAME: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 -s https://api.github.com/repos/v2fly/v2ray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | grep -v 'v5' | head -5 | awk '{print ""NR""":"$0}' | grep "${selectV2rayVersionType}:" | awk -F "[:]" '{print $2}')
		if [[ -n "${version}" ]]; then
			updateV2Ray "${version}"
		else
			echoContent red "\n ---> 输入有误，请重新输入"
...
xrayVersionManageMenu() {
	...
	if [[ "${selectXrayType}" == "1" ]]; then
		...
		echoContent yellow "3.如果回退的版本不支持当前的config，则会无法连接，谨慎操作"
		echoContent skyBlue "------------------------Version-------------------------------"
		curl -u USERNAME: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 -s https://api.github.com/repos/XTLS/Xray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | head -5 | awk '{print ""NR""":"$0}'
		echoContent skyBlue "--------------------------------------------------------------"
		read -r -p "请输入要回退的版本:" selectXrayVersionType
		version=$(curl -u USERNAME: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 -s https://api.github.com/repos/XTLS/Xray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | head -5 | awk '{print ""NR""":"$0}' | grep "${selectXrayVersionType}:" | awk -F "[:]" '{print $2}')
		if [[ -n "${version}" ]]; then
			updateXray "${version}"
		else
			echoContent red "\n ---> 输入有误，请重新输入"
...
updateXray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		if [[ -n "$1" ]]; then
			...
			version=$(curl -u USERNAME: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 -s https://api.github.com/repos/XTLS/Xray-core/releases | jq -r ".[]|select (.prerelease==${prereleaseStatus})|.tag_name" | head -1)
		fi
...
customCDNIP() {
	...
	echoContent yellow "# 注意事项"
	echoContent yellow "\n教程地址:"
	echoContent skyBlue "https://github.com/wangyu0829/v2ray-agent/blob/master/documents/optimize_V2Ray.md"
...
updateNginxBlog() {
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/wangyu0829/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/wangyu0829/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/wangyu0829/v2ray-agent/master/install.sh"
	else
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/wangyu0829/v2ray-agent/master/install.sh"
	fi
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/wangyu0829/v2ray-agent/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:wangyu0829"; then
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# 注意事项"
	echoContent yellow "任意门解锁详解，请查看此文章[https://github.com/wangyu0829/v2ray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
menu() {
	cd "$HOME" || exit
	echoContent red "\n=============================================================="
	echoContent green "作者:wangyu0829"
	echoContent green "当前版本:v2.5.78"
	echoContent green "Github:https://github.com/wangyu0829/v2ray-agent"
	echoContent green "描述:八合一共存脚本\c"
	showInstallStatus
	echoContent red "\n=============================================================="
	echoContent red "                        推广区                      "
	echoContent green "AFF捐赠：https://github.com/wangyu0829/v2ray-agent/blob/master/documents/donation_aff.md\n"
